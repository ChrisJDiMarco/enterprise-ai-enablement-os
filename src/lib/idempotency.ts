...
import { withTenant } from "./database.ts";
...
export async function withIdempotency<T>(
  ...
  // Each query runs in its own short tenant-context transaction so RLS applies,
  // while the handler runs OUTSIDE any transaction (no long-held connection).
  const existing = await withTenant(pool, organizationId, (client) =>
    client.query<{ response: T }>(
  ...
  const inserted = await withTenant(pool, organizationId, (client) =>
  ...
  if (inserted.rows.length === 0) {
    ...
    const winner = await withTenant(pool, organizationId, (client) =>
      client.query<{ response: T }>(
        "select response from idempotency_records where organization_id = $1 and scope = $2 and idempotency_key = $3",
        [organizationId, scope, key],
      ),

...
function firstRuntimeValue(secrets: RuntimeSecrets, env: RuntimeEnv, names: string[]) {
  return names.map((name) => secrets[name] || env[name]).find(Boolean) ?? "";
}
...
export function mergeRuntimeAISettings(
  ...
  return applyProviderRoutingDefaults({
    ...
    anthropicBaseUrl:
      ...
      routingSettings.anthropicBaseUrl ||
    ...
    deepseekBaseUrl:
      ...
      routingSettings.deepseekBaseUrl ||
    ...
    openrouterBaseUrl:
      ...
      routingSettings.openrouterBaseUrl ||

...
import { evaluateContextPolicy } from "@/lib/policy-engine";
...
export function retrieveContext(params: {
  ...
  const allowed = new Set(decision.allowedSourceIds);
  ...
  const results = params.sources
    .map((source) => {
      ...
      const score = Math.min(0.99, 0.45 + hits * 0.14 + (allowed.has(source.id) ? 0.2 : 0));
      return {
        sourceId: source.id,
        sourceName: source.name,
        score,
        snippet: `Source ${source.name} is ${source.classification} ${source.type} content owned by ${source.ownerDepartment}.`,
        permission: allowed.has(source.id) ? ("allowed" as const) : ("filtered" as const),
      };
    })

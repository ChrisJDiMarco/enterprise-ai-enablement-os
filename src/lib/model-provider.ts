...
import { selectModelForTask } from "@/lib/model-router";
...
function localGenerate(input: GenerateInput, route: ModelRouteDecision, latencyMs: number): GenerateOutput {
  ...
  const skillLabel = input.system.match(/You are the ([^.]+)\./)?.[1] ?? "configured Skill";
  const text = [
    `${skillLabel} completed a governed local Harness run.`,
    "The response was generated by the deterministic local runtime because no server-side provider is configured for this route.",

...
export function MetricCard({
  ...
  const content = (
    <>
      ...
      <div className="mt-2 truncate text-[25px] font-semibold leading-none tracking-[-0.02em] text-[var(--text)]">{value}</div>
    </>
  );
...
export function MiniMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-[var(--border)]/58 bg-[var(--surface)]/62 px-3 py-2.5 shadow-[var(--shadow-button)]">
      ...
      <div className="mt-1 min-w-0 truncate text-sm font-semibold text-[var(--text)]">{value}</div>
    </div>
  );
}

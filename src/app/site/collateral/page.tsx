      ...
      >
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {collateralAssets.map((asset) => (
            <article key={asset.id} className="rounded-lg border border-slate-200 bg-white p-5 shadow-[var(--shadow-card)]">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{asset.format}</span>
              </div>
              <h2 className="mt-5 text-lg font-semibold text-slate-950">{asset.title}</h2>
              ...
              <a
                ...
                className="mt-5 inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-[var(--primary)] px-4 text-sm font-semibold text-white transition hover:bg-[var(--primary-hover)]"
              >
                <Download size={16} />
                {asset.cta}
              </a>
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-8">
        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-[var(--shadow-card)]">
          <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center">
            ...
            <Link href="/" className="inline-flex h-11 items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-700 transition hover:border-slate-300">
              Open product
              <ArrowRight size={16} />
            </Link>
          </div>

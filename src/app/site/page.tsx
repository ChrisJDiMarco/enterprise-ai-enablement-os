...
export default function MarketingHomePage() {
  return (
    <main className="overflow-x-hidden">
      <section
        className="relative isolate min-h-[840px] overflow-hidden border-b border-[var(--border)] bg-[var(--surface)]"
        style={{
          backgroundImage:
            "linear-gradient(90deg, color-mix(in srgb, var(--surface) 99%, transparent) 0%, color-mix(in srgb, var(--surface) 96%, transparent) 38%, color-mix(in srgb, var(--surface) 72%, transparent) 66%, color-mix(in srgb, var(--surface) 30%, transparent) 100%), url('/marketing/enablement-os-command-center.png')",
          backgroundPosition: "center",
          backgroundSize: "cover",
        }}
      >
        <div className="absolute inset-x-0 top-0 h-px bg-[var(--surface)]/80" />
        <div className="relative z-10 mx-auto flex min-h-[840px] max-w-7xl flex-col justify-center px-5 pb-56 pt-24 sm:px-6 lg:px-8">
          <div className="max-w-4xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface)]/82 px-3 py-1.5 text-xs font-bold uppercase text-[var(--text-muted)] shadow-sm backdrop-blur">
              <span className="size-2 rounded-full bg-emerald-500" />
              Enterprise AI transformation command system
            </div>
            <h1 className="mt-7 max-w-5xl text-5xl font-semibold leading-[1.03] text-[var(--text)] sm:text-6xl lg:text-[72px]">
              Enterprise AI
              <br />
              Enablement OS
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href="/"
                className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-[var(--primary)] px-5 text-sm font-semibold text-[var(--primary-contrast)] shadow-[0_18px_50px_rgba(99,91,255,0.28)] transition hover:bg-[var(--primary-hover)]"
              >
                Open the OS
                <ArrowRight size={16} />
              </Link>
              <Link
                ...
                className="inline-flex h-12 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--surface)]/86 px-5 text-sm font-semibold text-[var(--text-muted)] shadow-sm backdrop-blur transition hover:border-[var(--border-strong)] hover:bg-[var(--surface)] hover:text-[var(--text)]"
              >
            ...
            <div className="mt-10 grid max-w-3xl gap-3 sm:grid-cols-3">
              {heroMetrics.map(([value, label]) => (
                <div key={value} className="rounded-lg border border-[var(--border)] bg-[var(--surface)]/84 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur">
                  <div className="text-2xl font-semibold text-[var(--text)]">{value}</div>
                  <div className="mt-1 text-xs leading-5 text-[var(--text-muted)]">{label}</div>
                </div>
              ))}
            </div>
          </div>
        </div>
        <div className="absolute bottom-8 left-1/2 z-20 hidden w-[min(1180px,calc(100vw-56px))] -translate-x-1/2 rounded-lg border border-[var(--border)] bg-[var(--surface)]/90 p-3 shadow-[0_24px_80px_rgba(15,23,42,0.14)] backdrop-blur-xl lg:block">
          <div className="grid grid-cols-4 gap-3">
            {enterpriseProof.map(({ title, body, icon: Icon }) => (
              <div key={title} className="flex gap-3 rounded-lg bg-[var(--surface-muted)] px-4 py-3">
                <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-[var(--surface)] text-[var(--primary)] shadow-sm">
                  <Icon size={17} />
                </span>
                <div>
      ...
      >
        <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-2 shadow-[0_24px_80px_rgba(15,23,42,0.07)]">
          <div className="grid gap-2 lg:grid-cols-10">
            {proofLoop.map((item, index) => (
              <div key={item} className="relative rounded-lg bg-[var(--surface-muted)] p-4">
                <div className="flex items-center justify-between gap-2">
                  <span className="flex size-8 items-center justify-center rounded-lg bg-[var(--surface)] text-sm font-semibold text-[var(--primary)] shadow-sm">
                    {index + 1}
                  </span>
                  {index < proofLoop.length - 1 ? <ArrowRight className="hidden text-[var(--text-soft)] lg:block" size={15} /> : <CheckCircle2 className="text-green-600" size={16} />}
                </div>
                <div className="mt-4 text-sm font-semibold leading-5 text-[var(--text)]">{item}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-8">
        <div className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-lg border border-white/10 bg-slate-950 p-7 text-white shadow-[0_24px_90px_rgba(15,23,42,0.18)] sm:p-8">
          ...
          <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[0_24px_80px_rgba(15,23,42,0.07)]">
            <div className="flex items-center justify-between gap-3 border-b border-[var(--border)] pb-4">
              <div>
                <div className="text-xs font-bold uppercase text-[var(--text-soft)]">Board-ready packet</div>
                <div className="mt-1 text-lg font-semibold text-[var(--text)]">Proof chain assembled</div>
              </div>
            ...
            <div className="mt-5 grid gap-3">
              {[
                ...
                <div key={title} className="grid grid-cols-[36px_1fr_auto] items-start gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] p-3">
                  <span className="flex size-9 items-center justify-center rounded-lg bg-[var(--surface)] text-emerald-600 shadow-sm">
                    <BadgeCheck size={17} />
                  </span>
                  <div>
                    <div className="text-sm font-semibold text-[var(--text)]">{title}</div>
      ...
      >
        <div className="grid gap-4 lg:grid-cols-3">
          {[
            ...
            <article key={item.title} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[0_18px_60px_rgba(15,23,42,0.05)]">
              <div className="flex size-11 items-center justify-center rounded-lg bg-[var(--text)] text-[var(--surface)]">
                <item.icon size={20} />
              </div>
      ...
      >
        <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[0_18px_60px_rgba(15,23,42,0.05)]">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {["Use case", "SkillSpec", "Trace", "Eval", "Approval", "ROI"].map((item) => (
                <span key={item} className="rounded-full bg-[var(--surface-subtle)] px-3 py-1.5 text-xs font-semibold text-[var(--text-muted)]">
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="grid gap-3">
            {buyerOutcomes.map((item, index) => (
              <div key={item.title} className="grid grid-cols-[40px_1fr] gap-4 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[0_18px_60px_rgba(15,23,42,0.05)]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 sm:px-6 lg:px-8">
        <div className="grid gap-4 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[0_24px_80px_rgba(15,23,42,0.07)] lg:grid-cols-4">
          {operatingModes.map(({ title, body, icon: Icon }) => (
            <div key={title} className="rounded-lg bg-[var(--surface-muted)] p-5">

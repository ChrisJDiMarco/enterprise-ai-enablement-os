...
test("planOrchestratorChat: model-generated action payloads are bounded by action type", async () => {
    ...
    new Response(
      JSON.stringify({
        output_text: JSON.stringify({
          ...
          actions: [
            {
              ...
              payload: { view: "billing-admin", secret: "sk-should-not-survive" },
              tone: "primary",
            },
            {
              type: "open_view",
            ...
            },
            {
              ...
              payload: { requestId: "tr-123", secret: "sk-should-not-survive" },
              tone: "primary",
            },
            {
          ...
          evidence: [{ label: "Status", value: "safe sk-should-not-survive-12345" }],
        }),
        status: "completed",
  ...
  try {
    ...
    assert.equal(evidenceSerialized.includes("sk-should-not-survive"), false);
    assert.equal(plan.autoActions.length, 0);
  } finally {
...
test("planOrchestratorChat: redacts user and history secrets before external model planning", async () => {
  ...
  try {
    const plan = await planOrchestratorChat({
      message: "Summarize this request from chris@example.com using api key sk-supersecretvalue123456.",
      history: [
        {
          role: "user",
...
test("compactWorkspaceForOrchestrator: redacts prompts, secrets, payloads, and oversized arrays before model context", () => {
  const compacted = compactWorkspaceForOrchestrator({
    ...
    selectedSkill: {
      ...
      openaiKey: "sk-secret-secret-secret",
    },
